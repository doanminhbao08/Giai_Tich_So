...
void tim_nghich_dao(double A[20][20], double nghich_dao[20][20], int n){
	if(n == 1){
		...
		if(mau(A, nghich_dao, n) == 0){
			cout << "Ma tran khong the nghich dao bang phuong phap vien quanh vi dinh thuc con cap " << n << " bang 0";	
			ngung_chay = 1;			
		}
		if(ngung_chay == 1){
			goto end;
...
int main(){
	...
	*/
	...
	xuat(A,n);
	if(A[0][0] == 0){
		cout << "Ma tran khong the nghich dao bang phuong phap vien quanh";
		goto end;	
	} 
